...
def calc_and_update(sess, Xc, Xr, Xp, layers, learning_rate):
    ...
    gradients = tf.gradients(loss, [h1_w, h1_b, h2_w, h2_b, o_w, o_b])

    # upgrade
    layers = [h1_w, h1_b, h2_w, h2_b, o_w, o_b]
    layers -= [learning_rate * g for g in gradients]
...
def train_epoch(sess, layers, Xc_train, Xr_train, Xp_train):
...
def train():
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(n_epochs):
            layers = train_epoch(sess, init_layers, Xc_train, Xr_train, Xp_train)
